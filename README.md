...
⚠️ ACTION REQUISE (une seule fois, pour permettre la connexion automatique de l'app)

1. Console Firebase → **Authentication**
2. Onglet **Sign-in method**
3. **Email/Password** → Activer → Enregistrer

> L'app utilise un compte partagé interne — les utilisateurs n'ont pas besoin de créer de compte. Ils entrent simplement le code PIN `1234` pour se connecter.

---

## 🔢 Code PIN de synchronisation

Le code PIN pour activer la synchronisation multi-appareils est :

```
1234
```

Tous vos appareils (téléphone, tablette, ordinateur) utilisent ce même code pour accéder aux mêmes données synchronisées en temps réel.

**Fonctionnement :**
- Ouvrir l'app sur n'importe quel appareil
- Dans la sidebar, entrer le code `1234` sur le pavé numérique
- Le point vert 🟢 confirme que la synchronisation est active
- L'app mémorise la connexion sur chaque appareil (auto-reconnexion)
- Pour verrouiller : bouton 🔒 dans la sidebar

**Sécurité :**
- 5 tentatives maximum avant blocage temporaire (5 minutes)
- Vibration sur mobile en cas de code incorrect
- Le PIN est transformé en mot de passe sécurisé côté Firebase — personne ne peut accéder aux données depuis la console Firebase sans ce mot de passe interne
